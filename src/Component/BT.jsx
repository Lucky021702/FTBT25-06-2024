...
//     }
//   };
//   const removeAllInlineStyles = (html) => {
//     // Define a regular expression to match any style attribute
//   const styleRegex = /style="[^"]*"/g;
//   // Remove the style attributes
//   let cleanedHtml = html.replace(styleRegex, '');
...
//                     const targetData =
//                       tmx[index]?.target || "No data available";
...
//                       } else if (percent >= 50) {
//                         return "orange"; // Yellow for percentages >= 50 and < 80
//                       } else {
...
//     ?.split(" ")
//     .map((tmWord, idx) => {
//       // Clean the word by removing any inline styles
//       const cleanedTmWord = removeAllInlineStyles(tmWord);
...
function BT() {
  ...
  const paginatedData = qcData?.Target?.slice(
    page * rowsPerPage,
  ...
  return (
    <div>
      ...
      {isQCSelected ? (
        <SourceTable uploadedData={uploadedData} />
      ) : qcData?.Target?.length !== 0 ? (
        <>
          <QCDataTable
            qcData={qcData}
            ...
            targetDataCk={targetDataCk}
            stripHtmlTags={stripHtmlTags}
            hideTmxColumn={hideTmxColumn}
            handleHide={handleHide}
            handleSaveBt={handleSaveBt}
            handleProjectDataUpdateBT={handleProjectDataUpdateBT}
            handleEditorChangeBt={handleEditorChangeBt}
            editableDataBt={editableDataBt}
        ...
        !isLoading && <div className="empty-message">Please load the file</div>
      )}
    </div>
  );
}
 
const SourceTable = ({ uploadedData }) => (
  <TableContainer component={Paper}>
    <Table aria-label="simple table" sx={{ minWidth: 650 }}>
      <TableHead style={{ position: "fixed" }}>
        <TableRow>
      ...
      <TableBody>
        {uploadedData.map((row, index) => (
          <TableRow key={index}>
            <TableCell style={{ fontSize: "1rem", width: "28%" }}>
              <div style={{ display: "flex" }}>
                <div>
                  <b>({index + 1})</b>
                </div>
  ...
  <div style={{ overflowX: "auto" }}>
    {shouldDisplay ? (
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          >
            <TableRow>
              ...
              <TableCell style={{ width: "15%" }}>Difference</TableCell>
              <TableCell style={{ width: "5%" }}>Match Percentage</TableCell>
              <TableCell style={{ width: "35%" }}>Target</TableCell>
              <TableCell style={{ width: "5%" }}>Action</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {paginatedData?.map((data, index) => {
              const globalIndex = page * rowsPerPage + index; // Calculate global index
              const targetData =
                tmx[globalIndex]?.target || "No data available";
              const Data = qcData.Target[globalIndex] || "No data available";
              const matchPercentageValue =
                tmx[globalIndex]?.["Match Percentage"] || "";
                    ...
                    <div style={{ display: "flex" }}>
                      ...
                      <span style={{ marginLeft: "0.5rem" }}>
                        <div>
                          {sourceData &&
                          sourceData.Target &&
                          sourceData.Target[globalIndex] ? (
                            <span
                              dangerouslySetInnerHTML={{
                                __html: sourceData.Target[globalIndex],
                              }}
                            />
                          ) : (
                            <span>No Data Available</span>
                    ...
                    >
                      <CKEditor
                        ...
                        data={
                          qcData?.Target?.[globalIndex] == ""
                            ? ""
                            : targetDataCk?.[globalIndex]
                            ? targetDataCk[globalIndex]
                            : tmx?.[globalIndex]?.source
                            ? tmx?.[globalIndex]?.source
                            : editableDataBt?.[globalIndex] || ""
                        }
                        onChange={(event, editor) =>
                          handleEditorChangeBt(event, editor, globalIndex)
                        }
                        config={{
                          toolbar: [
                            "bold",
                  ...
                  >
                    <Tooltip title="Save line" arrow>
                      <div
                        ...
                        onClick={() => {
                          handleSaveBt(globalIndex);
                          handleProjectDataUpdateBT(globalIndex);
                        }}
                      >
        ...
        {sourceData?.Target && <TablePagination
               rowsPerPageOptions={[10, 25, 50]}
              component="div"
              page={page}
              onPageChange={handleChangePage}
              rowsPerPage={rowsPerPage}
              count={sourceData?.Target?.length}
